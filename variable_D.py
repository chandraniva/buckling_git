...
def lag(sig,lamb, dpsi):
    return lamb + 1/lamb + dpsi[int(sig*nodes)]**2/8/lamb/E**2

def L(lamb,dpsi):
    return intg.quad(lag,0,1,args=(lamb,dpsi))[0]
...
def get_mu(D,lamb):
    ...
    return np.average(mu)

Ds = np.linspace(0,0.1,20)
mus = np.zeros_like(Ds)
lambds = np.zeros_like(Ds)
i=0
for D in Ds:
    mus[i], lambds[i] = get_mu(D,lamb)
    i += 1
