...
Es = np.linspace(Es1,Es2,grid_E)
l0s = np.sqrt(np.linspace(l01,l02,grid_l))
phase = np.zeros((grid_E,grid_l))
tol=1e-3*5
...
for E in Es:
    ...
    for l0 in l0s:
        ...
        plt.title("E="+str(round(E*100)/100)+";  \
                  l0^2="+str(round(l0**2*1000)/1000))
        plt.show()
        
        if i_ints:
            if amp2[min(i_ints)]<max(amp2[:min(i_ints)]):
                phase[ii,jj] = 1
                print("unfolding")
        else:
            if amp2[-1]<max(amp2):
...
plt.imshow(phase,origin='lower',extent=[l01,l02,Es1,Es2])
plt.ylabel(r'$\Xi$')
plt.xlabel(r'$l_0^2$')
plt.colorbar()
plt.savefig("phase diagram.png",dpi=400)
plt.show()
